public class Pachy.Widgets.MarkupView : Gtk.Box {
        ...
        traverse (root, (node) => {
            switch (node->name) {
                ...
                case "a":
                    ...
                    if (href != null) {
                        blockquote_handler_text += "<a href='" + Markup.escape_text (href) + "'>";
                        blockquote_handler (node);
    ...
    public static void default_handler (MarkupView v, Xml.Node* root) {
        switch (root->name) {
            ...
            case "a":
                ...
                if (href != null) {
                    v.write_chunk (@"<a href='$(Markup.escape_text (href))'>");
                    traverse_and_handle (v, root, default_handler);
                    v.write_chunk ("</a>");
                }
                break;
            ...
            case "text":
                if (root->content != null) {
                    v.write_chunk (Markup.escape_text (root->content));
                }
                break;
            default:
                warning (@"Unkown HTML tag: \"$(root->name)\"");
                traverse_and_handle (v, root, default_handler);
                break;
        }
    }
